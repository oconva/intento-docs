...
4. **Define IRS Endpoint**: Define an IRS endpoint with the configurations you want. You can define multiple IRS endpoints, each with its own configurations. Check the [IRS Endpoints](/irs-endpoints) page to learn more.
5. **Run the Server**: Once you have setup the IRS data source and defined the IRS endpoint, you can run the server to start the IRS endpoint.
...
- **Advanced Features**: Intento provides support advance features like query reformulation and response evaluation to improve the accuracy and performance of your intent recognition service.
