...
To learn more about configuring IRS endpoints to use a specific LLM model, check [here](/irs-endpoint).
...
import { setIRSDataSource, defineIRSEndpoint, runIRSServer } from "@oconva/intento";
...
Intento provides you with an easy way to define these IRS endpoints _qvikly_ using the **`defineChatEndpoint`** function. This is the real superpower of Intento. You can define a chat endpoint with support for chat history, authentication and response caching in just a few lines of code.

To learn more about different IRS endpoints that you can define, check the [IRS Endpoints](/chat-endpoints) section.
